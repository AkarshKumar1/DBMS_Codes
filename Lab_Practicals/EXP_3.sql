...
SELECT
     ...
     from employee
     WHERE
        ENAME LIKE '%S';   -- '%S' is used to find values that ends with the letter M.
...
SELECT 
	EMPNO,
	SAL + IFNULL('COMM',0 ) AS TOTAL_SALARY    -- IFNULL() is used to replace NULL values with a specified value.
	FROM  employee;
